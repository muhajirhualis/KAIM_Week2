...
class ReviewPreprocessor:
    ...
    def clean_text(self):
        """Clean review text, remove non-English reviews, and filter empty reviews."""
        # Print a header for this step [4/6]
        print("\n[4/6] Cleaning and filtering text...")
        
        # Define a robust language detection function (moved outside to be a standard helper)
        def is_english(text):
            if pd.isna(text) or text.strip() == "":
                return False # Treat empty/missing text as non-English for filtering purposes
            try:
                return detect(text) == 'en'
            except LangDetectException:
                return False # If detection fails (e.g., text too short), filter it out
                
        # Define the text cleaning function (moved outside for clarity)
        def clean_review_text(text):
            """Inner function to clean individual review text strings"""
            # If the text is NaN (missing), return empty string immediately
            if pd.isna(text):
                return ''
        ...
        # 1. Apply basic text cleaning
        print("Applying standard text cleaning...")
        self.df['review_text'] = self.df['review_text'].apply(clean_review_text)

        # 2. Filter out non-English texts
        print("Detecting and removing non-English reviews...")
        before_lang_filter_count = len(self.df)
        
        # Use boolean masking to keep only rows where is_english returns True
        english_mask = self.df['review_text'].apply(is_english)
        self.df = self.df[english_mask].copy()
        
        removed_lang_count = before_lang_filter_count - len(self.df)
        if removed_lang_count > 0:
            print(f"Removed {removed_lang_count} non-English reviews.")

        # 3. Filter empty/whitespace-only reviews that might remain
        before_count = len(self.df)
        # Keep only rows where the length of 'review_text' is greater than 0
        self.df = self.df[self.df['review_text'].str.len() > 0].copy()
        
        removed_empty = before_count - len(self.df)
        if removed_empty > 0:
            print(f"Removed {removed_empty} empty reviews.")

        # 4. Final statistics and feature engineering
        final_count = len(self.df)
        self.stats['empty_reviews_removed'] = removed_empty
        self.stats['non_english_reviews_removed'] = removed_lang_count
        self.stats['count_after_cleaning'] = final_count
        print(f"Total reviews remaining after cleaning: {final_count}")

        # Create a new column 'text_length' containing the character count
        # Note: Using loc is best practice when modifying a filtered DataFrame slice
        self.df.loc[:, 'text_length'] = self.df['review_text'].str.len()
    ...
    def remove_duplicates(self):
        ...
        # This creates a boolean Series: True for rows that are duplicates after the first occurrence
        column_to_check = 'user_name'

        duplicates_mask = self.df.duplicated(subset=[column_to_check], keep='first')
        
        # Count how many duplicates there are
        num_duplicates = duplicates_mask.sum()
        print(f"\nFound {num_duplicates} duplicates.")
        
        # Store initial stats before removal
        self.stats['duplicate_before'] = num_duplicates
        self.stats['total_rows_before'] = len(self.df)
        
        if num_duplicates == 0:
            print("No duplicates found in {column_to_check} Data is clean.")
            return
        ...
        self.stats['total_rows_after'] = len(self.df)
        self.stats['duplicates_removed'] = num_duplicates # Same as 'duplicate_before'
